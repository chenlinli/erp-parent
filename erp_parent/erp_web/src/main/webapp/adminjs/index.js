			...
			[
			 	{
					...
					[
						{"icon":"icon-sys","menuid":"101","menuname":"商品管理","url":"goods.html"}	,
						{"icon":"icon-sys","menuid":"102","menuname":"商品类型管理","url":"goodstype.html"}						
					]
			 	},
			 	{
			 		"icon":"icon-sys","menuid":"200","menuname":"人士管理","menus":
					[
						{"icon":"icon-sys","menuid":"201","menuname":"员工管理","url":"emp.html"}	,
						{"icon":"icon-sys","menuid":"202","menuname":"部门管理菜单","url":"dep.html"}						
					]
			 	}
...
function showName(){
	$.ajax({
		...
		success:function(rtn){
			//是否存在登录用户
			//访问json的key的方式有两种：对象.key/对象[key]
			if(rtn.success){
				$('#username').html(rtn['message']);//标签里面的内容
			}else{
				location.href="login.html";
			}
		}
